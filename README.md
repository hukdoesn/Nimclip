...
- 自定义全局快捷键、历史上限、保留时间和登录时启动；达到上限后自动删除最旧的未收藏记录，收藏不计入上限
- 暂停或恢复剪贴板记录
...
默认或用户自定义的全局快捷键都没有按键先后顺序。以默认的 <kbd>⌘</kbd> <kbd>⇧</kbd> <kbd>V</kbd> 为例，无论先按 V 还是先按修饰键，只要三个按键同时处于按下状态即可唤起 Nimclip；同一轮长按只会触发一次。
...
<p align="center">
  <img src="./docs/images/nimclip-settings-appearance-history.png" width="1120" alt="Nimclip 浅色与深色外观、快捷键和历史记录设置">
</p>
<p align="center"><sub>外观、快捷键与历史记录设置</sub></p>

<p align="center">
  <img src="./docs/images/nimclip-settings-ocr-general.png" width="1120" alt="Nimclip 图片文字识别、通用与直接粘贴设置">
</p>
<p align="center"><sub>图片文字识别、通用与直接粘贴设置</sub></p>
...
| 登录时启动 | 关闭 | 可在设置中开启 |
| 自动更新提醒 | 开启 | 可在设置中关闭，仍可从“关于”页手动检查 |

历史数量只限制未收藏记录。超过上限时，Nimclip 会自动删除最旧的未收藏记录及其本地图片文件；收藏记录不会被自动清理，因此收藏较多时总数可以超过所设置的上限。
...
默认情况下，Nimclip 启动约 4 秒后会检查一次 GitHub Releases，之后在运行期间每 10 分钟检查一次。只会提示比当前版本新的正式版本，自动检查失败时不会打扰使用。可以在“设置 → 通用”中关闭自动更新提醒；关闭后不会再执行后台定时检查，仍可从“关于”页手动检查。
